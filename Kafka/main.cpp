...
int main(int argc, char *argv[]) {
    ...
    std::string broker = "192.168.1.224:9092";
    std::string topic = "kafka-topic-test";

    {
        char test_data[100];
        strcpy(test_data, "hello world from rdkafka");

        ProducerKafka* producer = new ProducerKafka;
        if (PRODUCER_INIT_SUCCESS == producer->init_kafka(0, broker.c_str(), topic.c_str())) {
    ...
    {
        ConsumerKafka test;
        test.init_kafka(0, broker.c_str(), topic.c_str());
